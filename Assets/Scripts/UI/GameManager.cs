...
public class GameManager : MonoBehaviour
{
    ...
    public void NoticeRareSpawn(AllyData data, float rndValue, bool IsGotcha)
    {
        ...
        if (IsGotcha)
        {
            NoticeText.text = $"{rndValue.ToString()}% 확률로 [{data.Unit_Name} ({unitGrade})]를 획득하셨습니다.";
        }
        else
        {
            switch (data.Unit_Grade)
            {
                case 4:
                    NoticeText.text = $"유니크 3마리 조합으로 [{data.Unit_Name} ({unitGrade})]를 획득하셨습니다.";
                    break;
                case 5:
                    NoticeText.text = $"레전드 3마리 조합으로 [{data.Unit_Name} ({unitGrade})]를 획득하셨습니다.";
                    break;
            }
        }
